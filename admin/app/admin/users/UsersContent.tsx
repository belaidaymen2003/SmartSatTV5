              ...
              {rows.map((row) => (
                <tr key={row.id} className="border-b border-white/5 hover:bg-white/5">
                  ...
                  <td className="px-4 py-3">
                    <button onClick={() => toggleStatus(row.id)} className={`px-2.5 py-1 rounded-full text-xs font-medium border ${row.status === 'Approved' ? 'text-green-400 border-green-500/40 bg-green-500/10' : 'text-white/70 border-white/20 bg-white/5'}`}>{row.status}</button>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => openEditUser(row)} className="p-2 rounded-md bg-white/5 hover:bg-white/10" aria-label="Edit"><Edit2 className="w-4 h-4 text-blue-300" /></button>
                      <button onClick={() => (row.credits ? openEditCredits(row.id, row.credits) : openAddCredits(row.id))} className="p-2 rounded-md bg-white/5 hover:bg-white/10" aria-label="Credits"><Coins className="w-4 h-4 text-yellow-300" /></button>
                      {row.credits > 0 && (
                        <button onClick={() => deleteCredits(row.id)} className="p-2 rounded-md bg-white/5 hover:bg-white/10" aria-label="Reset credits"><Pencil className="w-4 h-4 text-emerald-300 rotate-90" /></button>
                      )}
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4" onClick={() => setCreditModal(null)}>
          <div className="w-full max-w-sm bg-black/30 border border-white/10 rounded-xl p-5 backdrop-blur-md" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid gap-3 text-sm">
              <input value={creditAmount} onChange={(e) => setCreditAmount(e.target.value)} placeholder="Amount" type="number" className="bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-white placeholder-white/30" />
              <div className="flex items-center justify-end gap-2 mt-1">
                ...
                <button onClick={saveCredits} className="px-3 py-2 rounded-md border border-orange-500 text-orange-400 hover:bg-orange-500/10">Save</button>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="w-full max-w-md bg-black/30 border border-white/10 rounded-xl p-5 backdrop-blur-md" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid gap-3 text-sm">
              ...
              <div className="grid grid-cols-2 gap-3">
                <select value={userModal.data.plan as any} onChange={(e) => setUserModal({ ...userModal, data: { ...userModal.data, plan: e.target.value as any } })} className="bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-white">
                  <option>Free</option>
                  <option>Basic</option>
                  <option>Premium</option>
                  <option>Cinematic</option>
                </select>
                <select value={userModal.data.status as any} onChange={(e) => setUserModal({ ...userModal, data: { ...userModal.data, status: e.target.value as any } })} className="bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-white">
                  <option>Approved</option>
                  <option>Banned</option>
                </select>
              </div>
              <div className="grid grid-cols-3 gap-3">
                <input value={userModal.data.comments as any} onChange={(e) => setUserModal({ ...userModal, data: { ...userModal.data, comments: Number(e.target.value) } })} type="number" min={0} placeholder="Comments" className="bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-white placeholder-white/30" />
                <input value={userModal.data.reviews as any} onChange={(e) => setUserModal({ ...userModal, data: { ...userModal.data, reviews: Number(e.target.value) } })} type="number" min={0} placeholder="Reviews" className="bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-white placeholder-white/30" />
                <input value={userModal.data.credits as any} onChange={(e) => setUserModal({ ...userModal, data: { ...userModal.data, credits: Number(e.target.value) } })} type="number" min={0} placeholder="Credits" className="bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-white placeholder-white/30" />
              </div>
              <div className="flex items-center justify-end gap-2 mt-1">
