...
import { Search, Filter, Eye, CheckCircle2, EyeOff, Trash2, Star, X } from 'lucide-react'
import AdminStore from '../../../lib/adminStore'
import type { AdminReview } from '../../../lib/adminStore'
...
export default function AdminReviewsPage() {
  ...
  const all = useMemo(() => AdminStore.getReviews(), [version])
  ...
  const approve = (id: number) => AdminStore.setReviewStatus(id, 'Approved')
  const hide = (id: number) => AdminStore.setReviewStatus(id, 'Hidden')
  const remove = (id: number) => { if (confirm('Delete review?')) AdminStore.deleteReview(id) }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold text-white">Reviews <span className="text-white/50 text-sm ml-2" suppressHydrationWarning>{total} Total</span></h1>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="ml-auto bg-black/20 backdrop-blur-sm border border-white/10 rounded-lg px-3 py-2 flex items-center gap-2 w-full md:w-80">
          ...
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search reviews..." className="bg-transparent text-white/80 text-sm w-full placeholder-white/40 focus:outline-none" />
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4" onClick={() => setActive(null)}>
          <div className="w-full max-w-md bg-black/30 border border-white/10 rounded-xl p-5 backdrop-blur-md" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="text-white/80 text-sm space-y-2">
              ...
              {active.content && <div className="mt-2 p-3 rounded bg-white/5 text-white">{active.content}</div>}
            </div>
          </div>
        </div>
      )}
