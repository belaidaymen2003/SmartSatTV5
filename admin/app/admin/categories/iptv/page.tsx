...
function HlsPlayer({ url }: { url: string }) {
  ...
  useEffect(() => {
    ...
    if (Hls.isSupported()) {
      ...
      hls.loadSource(url)
      hls.attachMedia(el)
      hls.on(Hls.Events.ERROR, (_, data) => { if (data.fatal) setError(data.details || 'Playback error') })
      el.play().catch(()=>{})
      return () => { hls.destroy() }
    } else {
...
export default function IPTVPage() {
  ...
  const openEdit = (ch: IPTVChannel) => {
    setEdit(ch)
    setForm({ name: ch.name || '', url: ch.url || '', logo: ch.logo || '', description: ch.description || '', category: ch.category || 'Live TV', cost: Number(ch.cost || 0) })
  }

  const saveEdit = async () => {
    if (!edit) return
    const payload = { id: edit.id, ...form }
    await fetch('/api/admin/categories/category', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
    setEdit(null)
    fetchChannels()
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="ml-auto bg-black/20 backdrop-blur-sm border border-white/10 rounded-lg px-3 py-2 flex items-center gap-2 w-full md:w-80">
          ...
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder={`Search in IPTV...`} className="bg-transparent text-white/80 text-sm w-full placeholder-white/40 focus:outline-none" />
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4" onClick={() => setEdit(null)}>
          <div className="w-full max-w-lg bg-black/30 border border-white/10 rounded-xl p-5 backdrop-blur-md" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-1 gap-3">
              ...
              <div className="text-sm text-white/70">Logo
